...
`, [requestEmail, requestEmail, requestEmail, requestEmail, Number(requestRow)]);
    ...
    getFeedListForDay(req: Request, res: Response) {
        let requestEmail = req.query.email;
        if (!requestEmail) return res.json({isSuccess: false, message: "이메일을 입력해주세요."});
        requestEmail = requestEmail.replace(/(\s*)/g, '');
        ...
        if (!requestScheduleDate) return res.json({isSuccess: false, message: "조회 일자를 입력해주세요."});
        requestScheduleDate = requestScheduleDate.replace(/(\s*)/g, '');
        if (requestScheduleDate.length != 8) return res.json({isSuccess: false, message: "조회 일자를 올바르게 입력해주세요."});
